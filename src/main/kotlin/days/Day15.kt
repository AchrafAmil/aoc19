...
private fun main() {
    ...
    println(day15Part1(ints))
    println(day15Part2(ints))
}

fun day15Part2(software: List<Long>): Int {
    //val map = exploreMap(software, 50_000)
    // re-using pre-explored map (since exploring is costy)
    val map = deserializeMap()
    visualizeMap(map)

    // Jargon: "to visit a point" = "to compare it's steps + 1 to neighbors' steps and set if smaller"
    val origin = map.filter { it.value == RepairDroid.Pixel.GOAL }.keys.first()
    val visited = mutableSetOf<XY>()
    var toVisit = mutableSetOf(origin)
    var stepsCount = 0

    while (visited.size < map.count { it.value == RepairDroid.Pixel.EMPTY }) {
        stepsCount++
        val nextToVisit = mutableSetOf<XY>()
        toVisit.forEach { point ->
            val neighbors = point.neighbors
                .filter { !visited.contains(it) && map[it] == RepairDroid.Pixel.EMPTY }
        ...
        println()
        visualizeOxygenSpread(map, visited)
        println()
    }
...
fun day15Part1(software: List<Long>): Int {
    //val map = exploreMap(software, 50_000)
    val map = deserializeMap()
    visualizeMap(map)
    ...
    // Jargon: "to visit a point" = "to compare it's steps + 1 to neighbors' steps and set if smaller"
    val origin = XY(0, 0)
    val goal = map.filter { it.value == RepairDroid.Pixel.GOAL }.keys.first()
    val lowestStepsMap = mutableMapOf(XY(0, 0) to 0)
    val visited = mutableSetOf<XY>()
    var toVisit = mutableSetOf(origin)
    ...
    while (!visited.contains(goal)) {
        ...
        toVisit.forEach { point ->
            val neighbors = point.neighbors
            val thisSteps = lowestStepsMap[point]!!
            neighbors
                .filter {
                    !visited.contains(it) && (map[it] == RepairDroid.Pixel.EMPTY || map[it] == RepairDroid.Pixel.GOAL)
                }
                .forEach { neighbor ->
                    val neighborSteps = lowestStepsMap[neighbor]
                    if (neighborSteps == null || thisSteps + 1 < neighborSteps) {
                        lowestStepsMap[neighbor] = thisSteps + 1
                    }
        ...
        println()
        visualizeDistances(map, lowestStepsMap)
        println()
    }

    return lowestStepsMap[goal]!!
}

private fun visualizeOxygenSpread(map: Map<XY, RepairDroid.Pixel>, visited: Set<XY>) {
    map.visualize { xy, pixel ->
        if (visited.contains(xy)) {
            'O'
        } else {
            visualizationChar(xy, pixel)
        }
    }
}

private fun visualizeDistances(map: Map<XY, RepairDroid.Pixel>, distancesMap: Map<XY, Int>) {
    map.visualize { xy, pixel ->
        distancesMap[xy]?.toString()?.last() ?: visualizationChar(xy, pixel)
    }
}

private fun visualizeMap(map: Map<XY, RepairDroid.Pixel>) {
    map.visualize { xy, pixel ->
        visualizationChar(xy, pixel)
    }
}

private fun visualizationChar(xy: XY, pixel: RepairDroid.Pixel?): Char {
    return when (xy) {
        XY(0, 0) -> 'A'
        else -> {
            when (pixel) {
                RepairDroid.Pixel.EMPTY -> '░'
                RepairDroid.Pixel.WALL -> '▓'
                RepairDroid.Pixel.GOAL -> 'X'
                RepairDroid.Pixel.UNKNOWN, null -> ' '
            }
        }
    }
}

private fun exploreMap(software: List<Long>, accuracy: Int = 1_000_000): Map<XY, RepairDroid.Pixel> {
    val droid = RepairDroid(software)

    repeat(accuracy) {
        val directions = Direction.values()
            .filter {
                val neighbor = droid.map.getOrDefault(droid.droidPosition.move(it), RepairDroid.Pixel.UNKNOWN)
                neighbor == RepairDroid.Pixel.UNKNOWN
            }
        ...
        println("step $it")
        droid.visualize()
        println("x=" + droid.map.keys.minBy { it.x })
        println("y=" + droid.map.keys.minBy { it.y })
        println()
    }
    return droid.map.toMap()
}

private fun deserializeMap(): Map<XY, RepairDroid.Pixel> {
            ...
            line.toCharArray()
                .mapIndexed { xIndex, char ->
                    val pixel = when (char) {
                        '░' -> RepairDroid.Pixel.EMPTY
                        '▓' -> RepairDroid.Pixel.WALL
                        'X' -> RepairDroid.Pixel.GOAL
                        else -> RepairDroid.Pixel.UNKNOWN
                    }
                    XY(xIndex + SERIALIZED_ORIGIN_X_OFFSET, yIndex + SERIALIZED_ORIGIN_Y_OFFSET) to pixel
                }
        }
        .flatten()
        .toMap()
...
private const val SERIALIZED_ORIGIN_X_OFFSET = -21
private const val SERIALIZED_ORIGIN_Y_OFFSET = -21
private val SERIALIZED_MAP =
    """
         ▓▓▓ ▓▓▓ ▓▓▓▓▓▓▓▓▓▓▓▓▓ ▓▓▓▓▓ ▓▓▓▓▓▓▓ ▓▓▓ 
        ▓░░░▓░░░▓░░░░░░░░░░░░░▓░░░░░▓░░░░░░░▓░░░▓
        ▓░▓░▓░▓░▓▓▓▓▓▓▓▓▓░▓▓▓░▓░▓▓▓░▓▓▓▓▓░▓░▓▓▓░▓
