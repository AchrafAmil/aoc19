...
private fun main() {
    val result = day3part2(INPUT)

    print(result)
}

fun day3part2(input: String): Int? {
    val firstWireMoves = input.split("\n")[0].split(",")
    val secondWireMoves = input.split("\n")[1].split(",")

    val firstWireCoordinates = firstWireMoves.accumulateMovesIntoCoordinatesWithSteps()
    val secondWireCoordinates = secondWireMoves.accumulateMovesIntoCoordinatesWithSteps()

    val intersect = firstWireCoordinates
        .mapNotNull { first ->
            val other = secondWireCoordinates.find { second ->
                first.x == second.x && first.y == second.y
            }
            other?.let { first + other }
        }
...
private fun List<String>.accumulateMovesIntoCoordinatesWithSteps(): List<WirePoint> {
    val coordinates = accumulateMovesIntoCoordinates()
    return coordinates.mapIndexed { index, coordinate -> coordinate.copy(steps = index + 1) }
}
...
fun day3part1(input: String): Int? {
    ...
    val firstWireCoordinates = firstWireMoves.accumulateMovesIntoCoordinates()
    val secondWireCoordinates = secondWireMoves.accumulateMovesIntoCoordinates()

    return firstWireCoordinates
        .intersect(secondWireCoordinates)
        .map { (x, y) -> abs(x) + abs(y) } // distances of all intersections
        .min()
}

private fun List<String>.accumulateMovesIntoCoordinates(): List<WirePoint> {
    val corners = listOf(WirePoint(0, 0)) + mapProvidingPreviousResult(WirePoint(0, 0)) { previousResult, move ->
        previousResult + move.toVectorCoordinates()
    }
    ...
    return corners
        ...
        .flatMap { (start, end) ->
            if (start.x == end.x) {
                val range = if ((start.y < end.y)) ((start.y + 1)..end.y) else ((start.y - 1) downTo end.y)
                range.toList().map { WirePoint(start.x, it) }
            } else {
                val range = if ((start.x < end.x)) ((start.x + 1)..end.x) else ((start.x - 1) downTo end.x)
                range.toList().map { WirePoint(it, start.y) }
            }
        }
}

private fun <T, R> List<T>.mapProvidingPreviousResult(
    initial: R,
    transform: (previousResult: R, element: T) -> R
): List<R> {
    val result = mutableListOf<R>()
    forEachIndexed { index, element ->
        val previousResult = if (index == 0) initial else result[index - 1]
        result.add(index, transform(previousResult, element))
    }
    return result
}

private fun String.toVectorCoordinates(): WirePoint {
    val movesCount = this.substring(1).toInt()
    return when (this[0]) {
        'R' -> WirePoint(movesCount, 0)
        'L' -> WirePoint(-movesCount, 0)
        'U' -> WirePoint(0, movesCount)
        'D' -> WirePoint(0, -movesCount)
        else -> throw IllegalStateException()
    }
}

data class WirePoint(val x: Int, val y: Int, val steps: Int = 0) {
    operator fun plus(other: WirePoint) = WirePoint(x + other.x, y + other.y, steps + other.steps)
}
